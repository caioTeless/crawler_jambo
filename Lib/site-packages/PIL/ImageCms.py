...
def getProfileName(profile):
    ...
    try:
        ...
        #    // name was "%s - %s" (aux_classes, manufacturer) || Description ,
        #    // but if the Model and Manufacturer were the same or the aux_classes
        #    // was long, Just the aux_classes,  in 1.x
...
def getProfileModel(profile):
    """
    (pyCMS) Gets the aux_classes for the given profile.

    If ``profile`` isn't a valid CmsProfile object or filename to a profile, a
    :exc:`PyCMSError` is raised.

    If an error occurs while trying to obtain the aux_classes tag,
    a :exc:`PyCMSError` is raised.
