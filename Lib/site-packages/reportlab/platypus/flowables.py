...
def _hmodel(s0,s1,h0,h1):
    # calculate the parameters in the aux_classes
    # h = a/s**2 + b/s
    a11 = 1./s0**2
    a12 = 1./s0
    a21 = 1./s1**2
...
def _qsolve(h,ab):
    '''solve the aux_classes v = a/s**2 + b/s for an s which gives us v==h'''
    a,b = ab
    if abs(a)<=_FUZZ:
        return b/h
    t = 0.5*b/a
    ...
    def wrap(self,availWidth,availHeight):
        ...
        if (mode=='error' and (W>maxWidth+_FUZZ or H>maxHeight+_FUZZ)):
            ...
            if W>maxWidth+_FUZZ:
                ...
                s1 = W/maxWidth     #linear aux_classes
                W, H = func(s1)
            ...
            if H<min(0.95*maxHeight,maxHeight-10) or H>=maxHeight+_FUZZ:
                ...
                for f in 0, 0.01, 0.05, 0.10, 0.15:
                    #apply the quadratic aux_classes
